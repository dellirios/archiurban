...
import { statusLabels, statusColors, formatCurrency, type Project } from '@/data/mockData';
import { cn } from '@/lib/utils';

const ClientPortal = () => {
  const { currentTenant, tenantProjects } = useApp();
  const { user, profile, signOut } = useAuth();
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(null);

  // Filter projects: match by client email from the logged-in user
  const clientEmail = user?.email || '';
  const clientProjects = tenantProjects.filter(p => {
    // Match projects where the client's email or name matches
    // For mock data, match by clientName containing part of the profile name
    const profileName = profile?.full_name?.toLowerCase() || '';
    return (
      p.clientName.toLowerCase().includes(profileName) ||
      profileName.includes(p.clientName.toLowerCase().split(' ')[0])
    );
  });

  // Fallback: if no match found, show all tenant projects (for demo)
  const visibleProjects = clientProjects.length > 0 ? clientProjects : tenantProjects;
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
            ...
            <span className={cn('text-xs px-3 py-1 rounded-full font-medium flex-shrink-0', statusColors[currentProject.status])}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-secondary/50 rounded-lg p-3">
              ...
              <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden mt-2">
                <div className="h-full bg-primary rounded-full" style={{ width: `${currentProject.progress}%` }} />
              </div>
            </div>
            <div className="bg-secondary/50 rounded-lg p-3">
            ...
            <div className="bg-secondary/50 rounded-lg p-3">
              ...
              <p className="text-sm font-semibold text-foreground mt-1">
                {new Date(currentProject.endDate).toLocaleDateString('pt-BR')}
              </p>
            </div>
            <div className="bg-secondary/50 rounded-lg p-3">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="text-sm font-semibold text-foreground mb-6">
            Diário de Obra
            <span className="text-xs text-muted-foreground font-normal ml-2">({currentProject.photos.length} fotos)</span>
          </h3>
